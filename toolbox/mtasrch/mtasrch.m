%mtasrch  More and Thuente Arc Search
%
...
%
function [x f g stp info nfev] = mtasrch(fcn,x,f,g,arc,stp,...
                                         ncur,ftol,gtol,xtol,...
                                         stpmin,stpmax,maxfev,...
                                         fid,bisect)

  %
  % 2010-02-06 (nwh) created bdmt
  % 2010-02-16 (nwh) modified to bdmt2 for efficiency
  %                  this version does not require the extra function
  %                  evaluation.
  % 2010-02-23 (nwh) changed termination flags
  % 2010-02-25 (nwh) added code to print information each iteration
  % 2010-04-06 (nwh) modifying code to handle arcs and negative curvature
  % 2010-11-15 (nwh) changed some comments
  % 2011-09-06 (nwh) changed to mtasrch, added interpolation for step selection
  %
  ...
  while(1)
    ...
    if bracket && bisect
      ...
      stp = mtastep(mstx,mfx,mdx,msty,mfy,mdy,stp,mfp,mdp,bracket,stmin,stmax);
    end
